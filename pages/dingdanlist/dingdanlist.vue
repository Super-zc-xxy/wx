		...
		<view class="dl_main">
			<view v-for="(item,index) in rwdata" :key="index" class="dl_info">
				...
				<view class="info_ddh_price">
					<view class="info_ddh_price_left">总金额：￥{{item.money}}.00</view>总金额：￥{{item.money}}.00
				</view>
				<!-- 取消订单 -->
				<view class="info_ddh_submit">
					<view class="info_ddh_submit_button" @click="startrw(index)" v-if="item.state == `已接单` ">开始任务</view>
					<view class="info_ddh_submit_button" @click="endrw(index)" v-if="item.state == `进行中` ">结束任务</view>
					<view class="info_ddh_submit_button" @click="quxiao(index)" v-if="item.state == `未进行` || item.state == `已接单` || item.state == `进行中` ">
						取消任务
					</view>
				</view>
			</view>
				...
				if(index == 0){
					...
					this.$request('workList',{
						...
						for(let j=0;j<this.rwdata.length;j++){
							if(this.rwdata[j].state == 1){
								this.rwdata[j].state = "未进行"
							}
							if(this.rwdata[j].state == 2){
								this.rwdata[j].state = "已取消"
							}
						}
